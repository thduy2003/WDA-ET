        ...
        <div className="flex gap-3">
            ...
            <div className="w-full py-4 px-6 flex flex-col gap-2 border border-solid border-[#EAEAEA] rounded-[8px]">
                <div className="sm:flex justify-between items-center">
                    <div className="text-[#141716] font-semibold max-sm:text-[15px] text-[20px]">{data?.name ?? user.name}</div>
                    <div className="text-[#141716] font-semibold max-sm:text-[10px] text-[14px]">{format(data.date_post)}</div>
                </div>
                <p> {data.content}</p>
            </div>
        </div>
